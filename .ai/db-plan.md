...
- `user_id uuid NOT NULL` *(Supabase Auth: `auth.users.id`)*
- `joined_at timestamp without time zone NOT NULL DEFAULT (now() at time zone 'utc')`

Ograniczenia:
- `PRIMARY KEY (company_id, user_id)`
- `UNIQUE (user_id, company_id)` *(dla FK z `user_profiles`)*
...
- `user_id uuid PRIMARY KEY` *(Supabase Auth: `auth.users.id`)*
- `default_company_id uuid NULL`
- `created_at timestamp without time zone NOT NULL DEFAULT (now() at time zone 'utc')`

Ograniczenia:
- `FOREIGN KEY (user_id, default_company_id) REFERENCES company_members(user_id, company_id) ON DELETE RESTRICT` *(wymusza, że domyślna firma jest firmą, do której user należy)*
...
- `file_name text NULL`
- `uploaded_by uuid NULL` *(zwykle `auth.uid()` podczas uploadu)*
- `created_at timestamp without time zone NOT NULL DEFAULT (now() at time zone 'utc')`
...
- `amount_book_cents bigint NOT NULL CHECK (amount_book_cents >= 0)` *(zawsze w `companies.base_currency`)*

Daty i kategoryzacja:
- `date_due date NOT NULL`
- `direction text NOT NULL CHECK (direction IN ('INFLOW','OUTFLOW'))`
- `time_slot text NOT NULL CHECK (time_slot ~ '^(IB|[0-9]{4})$')` *(IB albo YYWW)*
...
- `auth.users (1) -> (N) company_members` *(po `user_id`)*
- `auth.users (1) -> (1) user_profiles` *(po `user_id`)*
...
- `accounts (1) -> (0..N) transactions` *(opcjonalny FK, `account_id` może być NULL)*

- `companies (1) -> (N) scenarios`
- `imports (1) -> (N) scenarios` *(scenarios.import_id wskazuje import będący „najnowszym” dla `dataset_code`)*
...
### 3.1. `company_members`
- `PRIMARY KEY (company_id, user_id)` *(wspiera RLS EXISTS po company_id)*
- `UNIQUE (user_id, company_id)` *(wspiera zapytania „jakie firmy ma user” oraz FK z `user_profiles`)*

### 3.2. `imports`
- `INDEX imports_company_dataset_created_idx (company_id, dataset_code, created_at DESC)` *(wybór „najnowszego importu paczki”)*
...
- `INDEX transactions_company_project_idx (company_id, project)` *(BTREE; działa też dla NULL, ale selektywność zależy od danych)*
...
*(Można to zmaterializować jako SQL function, ale nie jest wymagane.)*
...
*(Najprościej: policy z `EXISTS (SELECT 1 FROM scenarios s WHERE s.id = scenario_id AND s.company_id = scenario_overrides.company_id AND s.status='Draft' AND s.deleted_at IS NULL AND EXISTS (...company_members...))`.)*
...
**Cel:** Scalenie transakcji z override’ami do „effective_*”.

Minimalny kontrakt kolumn:
- `company_id`
- `scenario_id`
- `transaction_id` *(opcjonalnie: `transactions.id` jako referencja debugowa)*
- `dataset_code`
- `flow_id`
- `direction`
- `time_slot`
...
- `week_index integer` *(0 dla IB, 1..N dla pozostałych)*
- `week_start_date date` *(opcjonalnie, zależnie od UI)*
- `inflow_total_book_cents bigint`
- `outflow_total_book_cents bigint`
- `inflow_top5 jsonb` *(lista 5 największych pozycji; pola: flow_id, amount, counterparty/description)*
- `outflow_top5 jsonb`
...
- `company_id`, `scenario_id`
- `as_of_date date`
- `delta_book_cents bigint` *(signed: inflow dodatni, outflow ujemny)*
